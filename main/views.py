...
def create_auction(req):
    ...
    if req.method == "POST":
        ...
        begin = datetime.strptime(req.POST["begin"], '%Y-%m-%dT%H:%M')
        end = datetime.strptime(req.POST["end"], '%Y-%m-%dT%H:%M')
...
def join_auction(req):
    ...
    auction_id = req.POST["auction_id"]
